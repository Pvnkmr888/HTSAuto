...
public class VerifyEnabledElement extends Keyword {
	...
	public VerifyEnabledElement(TestCases testCase, TestCaseInputs inputs,ArrayList<String> expectedLocator) {
	...
	@Override
	@KeywordStep(gherkins = "^verify (.*) is enabled$")
		...
		try{
			if(testCase.isTestSuccessful()){
				...
				case "CAMERA MICROPHONE":{
					...
					if (ele.equalsIgnoreCase("ON")||ele.equalsIgnoreCase("true")) {
						flag=true;
						Keyword.ReportStep_Pass(testCase,expectedLocator.get(0)+" is enabled");
					}else{
						Keyword.ReportStep_Fail(testCase, FailType.FUNCTIONAL_FAILURE, expectedLocator.get(0)+" not enabled");
						flag=false;
					}
					break;
				}
